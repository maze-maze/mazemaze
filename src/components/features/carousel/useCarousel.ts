...
function getToIndex(length: number, from: number, to: number) {
  const indexes = [...new Array(length)].map((_, i) => i)
  const fromToEnd = indexes.slice(from)
  const startToFrom = indexes.slice(0, from)
  return [...fromToEnd, ...startToFrom].indexOf(to)
}

export function useCarousel(setActiveIndex: Dispatch<SetStateAction<number>>, sliderOption: EmblaOptionsType, plugins?: EmblaPluginType[]) {
  const [sliderRef, sliderAPI] = useEmblaCarousel(sliderOption, plugins)

  const handleNextScroll = () => {
    if (!sliderAPI || !sliderAPI.canScrollNext())
      return
  ...
  const handlePrevScroll = () => {
    if (!sliderAPI || !sliderAPI.canScrollPrev())
      return
  ...
  const handleToRightScroll = (toIndex: number) => {
    ...
    for (const _ of new Array(scrollTimes)) {
      sliderAPI.scrollNext()
    }
