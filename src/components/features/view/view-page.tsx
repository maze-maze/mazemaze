      ...
      >
        <CarouselContent className="h-screen">
          {Array.from({ length: 4 }).map((_, index) => (
            <CarouselItem key={index} className="h-full w-full flex flex-col items-center gap-8 pt-30 relative">
              <p className="text-2xl font-bold p-4 bg-black/30 backdrop-blur-md rounded-2xl">{items[index].title}</p>
              <div className={cn(items[index].bg, 'size-50 rounded-3xl mx-auto flex items-center justify-center')}>
                <Image
                  src="/lama.png"
                  alt=""
                  width={150}
              ...
              <Image
                ...
                height={200}
              />
              <div className="absolute right-3 bottom-40 z-50 flex flex-col gap-4">
                <Button size="icon" variant="outline" className="rounded-full bg-black/30 backdrop-blur-md size-13">
