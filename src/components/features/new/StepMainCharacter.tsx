...
export default function MainCharacterSelector({
  ...
  // personalitiesがセットされた後に2番目に遷移 (自分カード)
  useEffect(() => {
    if (emblaApi) {
      // personalitiesの数が0でも自分カードは常に表示
      emblaApi.scrollTo(0)
      setSelectedIndex(0)
    }
  }, [emblaApi]) // personalities.length の依存を削除
    ...
    <div className={`flex flex-col overflow-hidden  items-center w-full min-h-screen h-full ${gradient || ''}`}>
          ...
          <div className="p-4">
            <div className="embla" ref={emblaRef}>
              <div className="embla__container flex gap-4">
                {[selfCard, ...personalities].map((personality, idx) => (
                  <div
                    key={idx}
            ...
            <div className="w-full mt-8 flex justify-center">
              <div className="flex justify-center gap-2 w-fit px-3 py-2 rounded-full opacity-40 bg-[#BFBFBF]">
                {[selfCard, ...personalities, {} as Personality].map((_, idx) => (
                  <button
                    key={idx}
      ...
      <style jsx global>
        ...
        @keyframes expandCard {
          ...
          100% { transform: scale(1); opacity: 1; border-radius: 0px; }
        }
      `}
      </style>
    </div>
